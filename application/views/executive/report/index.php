<div class="uk-container uk-container-center">
	<div class="uk-grid">
		...
		<div class="tm-main uk-width-medium-3-4 uk-margin-top uk-margin-bottom">
			<form action="<?php echo base_url('executive/report');?>" method="post" name="adminForm" class="uk-form">
				<div class="uk-clearfix"><h2>รายงานการปฏิบัติหน้าที่กิจกรรมโฮมรูมของครูที่ปรึกษา</h2></div>
				<hr/>
				<div class="uk-text-center">
					<select name="executive_filter_semester" class="uk-width-small-1-1 uk-width-large-1-4" onchange="document.adminForm.submit();">
						<option value="">- กรุณาเลือกภาคเรียน -</option>
						<?php
                            foreach ($filter_semester as $semester) {
                                ?>
							<option value="<?php echo $semester->id; ?>" <?php echo set_select('executive_filter_semester', $semester->id); ?>><?php echo $semester->name; ?></option>
						<?php
                            }
                        ?>
					</select>
					<select name="executive_filter_major" class="uk-width-small-1-1 uk-width-large-1-4" onchange="document.adminForm.submit();">
						<option value="">- กรุณาเลือกสาขาวิชา -</option>
						<?php
                            foreach ($filter_major as $major) {
                                ?>
							<option value="<?php echo $major->id; ?>" <?php echo set_select('executive_filter_major', $major->id); ?>>- <?php echo $major->major_name; ?></option>
						<?php
                            }
                        ?>
					</select>
					<select name="executive_filter_type" class="uk-width-small-1-1 uk-width-large-1-4" onchange="document.adminForm.submit();">
						<option value="">- กรุณาเลือกรูปแบบรายงาน -</option>
						<option disabled value="9" <?php echo set_select('executive_filter_type', '9'); ?>>- แบบรายงาน 9 สัปดาห์</option>
						<option disabled value="18" <?php echo set_select('executive_filter_type', '18'); ?>>- แบบรายงาน 18 สัปดาห์</option>
						<option value="current" <?php echo set_select('executive_filter_type', 'current'); ?>>- แบบรายงานจนถึงสัปดาห์ปัจจุบัน</option>
					</select>
				</div>
				<hr/>
